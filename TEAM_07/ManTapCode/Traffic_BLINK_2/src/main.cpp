...
void processDisplay()
{
  ...
  static unsigned long ledStartTime = millis();
  ...
  uint32_t remaining = waitTime[idxLED] > elapsed ? waitTime[idxLED] - elapsed : 0;
  
  if (remaining == 0)
  {
    idxLED = (idxLED + 1) % 3;
    ledStartTime = millis();
    remaining = waitTime[idxLED];
  }
  
  if (valueButtonDisplay == HIGH)
  {
    int displayValue = remaining / 1000;
